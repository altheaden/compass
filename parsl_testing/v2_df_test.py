...
def main():
    ...
    '''
    Tasks dictionary explanation:
    * Each first level key in tasks is a number which represents the
        task number. i.e., task 0 has key 0.
    * Each task contains three elements:
        > A list representing the task's inputs
        > A list representing the task's outputs
        > An app future generated by calling input_data()
    * inputs[] and outputs[] for each task each have a single file
    * data_futures[] is a list that has each data future for each
        task sorted by task number. As each app future is created, its 
        data future is added to the list. Only the data futures that are
        needed by analyze() would need to be in this list.
    * Finally analyze() is sent the list of data futures and waits until 
        they are all done to begin execution.
    ...
    start_timer = time.time()  # For testing parallelism
